...
methrix_differential <- function(m, pheno = NULL, smooth = FALSE, s_span = 500, delta = 0.1, p = 0.05,
  ...
  if(!chrWise & parallel) {
    stop("Parallel computing can just be executed in a chromosome wise manner.")
  }
  ...
  if(!chrWise & !parallel){
    ...
    dml_test <- DSS::DMLtest(BSobj = methrix_bs, group1 = class1, group2 = class2, smoothing = smooth, smoothing.span =  s_span)
   # print(head(dml_test))
  }else if(parallel){
    ...
    for(i in chroms){
     ...
     methrix_work <- methrix::subset_methrix(m = m, contigs = i)
     methrix_bs <-  methrix::methrix2bsseq(m = methrix_work)
     dml_classI_classII[[i]] <- DSS::DMLtest(BSobj = methrix_bs, group1 = class1, group2  = class2, smoothing = smooth, smoothing.span = s_span)}
    # combine the DMLs and adjust the p-values
    dml_test <- do.call("rbind", dml_classI_classII)
    dml_test$fdr<-p.adjust(dml_test$pval, method = "BH")
    }
    ...
    sigDML <- DSS::callDML(DMLresult = dml_test, delta = delta, p.threshold = p)
    #print(head(sigDML))
    # call DMRs
    DMRs <- DSS::callDMR(DMLresult = dml_test, delta = delta, p.threshold = p, minlen = minlen,minCG = minCG, dis.merge = mergeDis, pct.sig = pct.sig)
