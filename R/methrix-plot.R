...
plot_violin <- function(m, ranges = NULL, n_cpgs = 25000, pheno = NULL, col_palette = "RdYlGn") {
    ...
    if (!is.null(ranges)) {
        ...
        ids = sample(x = seq_along(m), replace = FALSE, size = min(n_cpgs, nrow(m)))
    ...
    p <- ggplot2::ggplot(plot.data, ggplot2::aes(x = variable, y = Meth, fill = variable)) +
        ggplot2::geom_violin(alpha = 0.8) + ggplot2::theme_classic(base_size = 14) +
        ggplot2::scale_fill_brewer(type = "div", palette = col_palette) + ggplot2::xlab(pheno) +
        ggplot2::ylab(expression(beta * "-Value")) + theme(axis.title.x = element_blank(),
...
plot_density <- function(m, ranges = NULL, n_cpgs = 25000, pheno = NULL, bw.adjust = 2,
    col_palette = "RdYlGn") {
    ...
    p = ggplot2::ggplot(plot.data, ggplot2::aes(Meth, color = variable)) + geom_density(lwd = 1,
        position = "stack") + ggplot2::theme_classic() + ggplot2::xlab("Methylation") +
        ggplot2::theme_classic(base_size = 14) + ggplot2::scale_color_brewer(type = "div",
        palette = col_palette) + ggplot2::xlab(expression(beta * "-Value")) + theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 12, colour = "black"), axis.text.y = element_text(size = 12,
            colour = "black"), axis.title.y = element_blank(), legend.title = element_blank())
...
#' @export
#'
methrix_pca <- function(m, var = "top", top_var = 1000, ranges = NULL, pheno = NULL,
    do_plot = TRUE, n_pc = 2) {
    ...
    if (is.null(top_var)) {
        ...
        if (var_select == "rand") {
            if (!is.null(ranges)) {
                ...
                ids = sample(x = seq_along(meth_sub), replace = FALSE, size = min(top_var,
                  nrow(meth_sub)))
            } else {
    ...
    if (do_plot) {
        par(bty = "n", mgp = c(2.5, 0.5, 0), mar = c(3, 4, 2, 2) + 0.1, tcl = -0.25,
            las = 1)
        ...
        axis(side = 4, at = pretty(c(0, 1)) * meth_pca$sdev[1]^2/sum(meth_pca$sdev^2),
            labels = pretty(c(0, 1)))
        legend("topright", col = c("red", "black"), lty = 1, c("Per PC", "Cumulative"),
            bty = "n")
        lines(x = c(length(meth_pca$sdev), n_pc, n_pc), y = c(cum_var[n_pc], cum_var[n_pc],
            0), lty = 3)
...
#' plot_pca(mpc)
#' @export
plot_pca = function(pca_res, m = NULL, col_anno = NULL, shape_anno = NULL, pc_x = "PC1",
    pc_y = "PC2", show_labels = FALSE) {
    ...
    if (all(c("color_me", "shape_me") %in% colnames(pca_res))) {
        pca_gg = ggplot(data = pca_res, aes(x = X, y = Y, color = color_me, shape = shape_me,
        ...
        pca_gg = ggplot(data = pca_res, aes(x = X, y = Y, color = color_me, label = row_names)) +
            geom_point(size = 3) + xlab(pc_x) + ylab(pc_y) + labs(color = col_anno) +
            scale_color_brewer(palette = "Dark2")
    } else if ("shape_me" %in% colnames(pca_res)) {
        pca_gg = ggplot(data = pca_res, aes(x = X, y = Y, shape = shape_me, label = row_names)) +
            geom_point(size = 3) + xlab(pc_x) + ylab(pc_y) + labs(shape = shape_anno)
    } else {
        pca_gg = ggplot(data = as.data.frame(pca_res), aes(x = X, y = Y, label = row_names)) +
            geom_point(size = 3, fill = "black", color = "gray70") + xlab(pc_x) +
            ylab(pc_y)
    }
...
plot_coverage <- function(m, type = c("hist", "dens"), pheno = NULL, perGroup = FALSE,
    ...
    # On an average a matrix of 28e6 rows x 10 columns, sizes around 2.4 GB. Copying,
    # and melting would double the memory consumption. We should think of something
    # else.

    if (nrow(m) > size.lim) {
        cat("The dataset is bigger than the size limit. A random subset of the object will be used that contains ~",
            size.lim, " observations. \n")
        n_rows <- trunc(size.lim/nrow(m@colData))
        sel_rows <- sample(seq_along(m@elementMetadata), size = n_rows, replace = FALSE)
    ...
    ## melt the object to a long format add support for DelayedMatrix
    ...
    if (!perGroup) {
        if (type == "dens") {
            p <- ggplot2::ggplot(plot.data, aes(value, color = variable)) + ggplot2::geom_density(alpha = 0.5,
                adjust = 1.5, lwd = 1, position = "stack") + ggplot2::theme_classic() +
                ggplot2::xlab("Coverage") + ggplot2::scale_color_brewer(type = "div",
                palette = col_palette)
        ...
        if (type == "dens") {
            p <- ggplot2::ggplot(plot.data, ggplot2::aes(value, color = variable)) +
                ggplot2::geom_density(alpha = 0.6, adjust = 1.5, lwd = 1, position = "stack") +
                ggplot2::theme_classic() + ggplot2::xlab("Coverage") + ggplot2::labs(fill = "Groups") +
            ...
            p <- ggplot2::ggplot(plot.data, ggplot2::aes(value, color = variable)) +
                ...
                ggplot2::labs(fill = "Groups") + ggplot2::scale_color_brewer(type = "div",
                palette = col_palette)
            # print(p)
        }
    }
...
plot_stats = function(plot_dat, what = "M", stat = "mean", ignore_chr = NULL, samples = NULL,
    ...
    if ("Chromosome" %in% colnames(plot_dat)) {
        ...
        plot_dat_gg = ggplot(data = plot_dat, aes(x = Chromosome, y = measurement)) +
            geom_errorbar(aes(ymin = sd_low, ymax = sd_high), col = "gray70") + geom_point(col = "maroon") +
            facet_wrap(~Sample_Name, nrow = n_row, ncol = n_col) + theme_minimal(base_size = 12) +
            theme(axis.title.x = element_blank(), axis.text.x = element_text(hjust = 1,
                size = 10, colour = "black"), axis.text.y = element_text(size = 10,
                colour = "black"), axis.title.y = element_blank())
        ...
        plot_dat_gg = ggplot(data = plot_dat, aes(x = Sample_Name, y = measurement)) +
            ...
            ymax = sd_high), col = "gray70") + geom_point(col = "maroon") + theme_minimal(base_size = 12) +
