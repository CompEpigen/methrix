...
# Process samples in batches. Batches are processed in vectorized manner (ideal
# for large number of samples)
merge_vect_code_batch = function(files, col_idx, batch_size, ideal = FALSE, col_data = NULL, 
    genome = NULL) {
    ...
    for (i in seq_along(batches)) {
        ...
        if (ideal) {
            if (i == 1) {
                ...
                colnames(cov_mat_final) = colnames(beta_mat_final) = c("chr", "start", 
                  samp_names)
            } else {
                cov_mat = data.frame(lapply(bdgs, function(x) x[, .(cov)]), stringsAsFactors = FALSE)
                beta_mat = data.frame(lapply(bdgs, function(x) x[, .(beta)]), stringsAsFactors = FALSE)
            ...
            if (i == 1) {
                ...
                cov_mat_final = data.table::dcast(bdgs, chr + start ~ sample, value.var = "cov", 
                  fill = 0)
            } else {
                beta_mat_final = merge(beta_mat_final, data.table::dcast(bdgs, chr + 
                  start ~ sample, value.var = "beta"), by = c("chr", "start"), all = TRUE)
                cov_mat_final = merge(cov_mat_final, data.table::dcast(bdgs, chr + 
                  start ~ sample, value.var = "cov", fill = 0), by = c("chr", "start"), 
                  all = TRUE)
            }
        }
    }
...
merge_non_vect_code = function(files, col_idx, coldata, verbose = TRUE, ideal = FALSE, 
    genome = NULL) {
        ...
        if (ideal) {
            ...
            if (i == 1) {
                ...
                beta_mat = merge(beta_mat, b[, .(chr, start, beta)], by = c("chr", 
                  "start"), all = TRUE)
                cov_mat = merge(cov_mat, b[, .(chr, start, cov)], by = c("chr", "start"), 
                  all = TRUE)
                colnames(beta_mat)[ncol(beta_mat)] = colnames(cov_mat)[ncol(cov_mat)] = rownames(coldata)[i]
            }
        }
