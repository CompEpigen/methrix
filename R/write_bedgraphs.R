...
write_bedgraphs = function(m, output_dir = NULL, rm_NA = TRUE, n_thr = 4, make_BigWig = FALSE,
  ...
  op_bdgs = lapply(seq_len(nrow(colData(m))), function(i){
                ...
                data.table::fwrite(x = mat_i, file = paste0(output_dir, "/", rownames(colData(m))[i], ".bedGraph"),
                                   sep = "\t", col.names = FALSE, nThread = n_thr)
                paste0(output_dir, "/", rownames(colData(m))[i], ".bedGraph")
              })
  cat("Finshed writing bedGraphs..\n")
  cat("----------------------\n")
  ...
  if(make_BigWig){
    ...
    chr_lens = data.table::copy(x = m@metadata$ref_CpG)
    #chr_lens[,start := 1]
    ...
    cat("Finshed converting bedGraphs\n")
  }
}
