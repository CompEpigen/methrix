...
#' }
#' @export
methrix_report = function(meth, output_dir = NULL, recal_stats = FALSE, plot_beta_dist = TRUE, 
    beta_nCpG = 10000, n_thr = 4) {
    ...
    if (file.exists(of3)) {
        cat("File already present. Skipping step 3..\n")
    } else {
        if (recal_stats) {
            ...
            non_cov_tbl = merge(non_cov_tbl, contig_nCpGs, by = "chr", all.x = TRUE)
    ...
    # Density plot data
    cat(paste0("Step 5 of 5: beta value distribution\n"))
    if (plot_beta_dist) {
        dens_files = list.files(path = output_dir, pattern = "*_density\\.tsv\\.gz$")
        if (length(dens_files) == nrow(colData(meth))) {
            cat("Files already present. Skipping step 5..\n")
        } else {
            if (!exists(x = "na_vec")) {
                na_vec = apply(get_matrix(meth, type = "M"), MARGIN = 1, anyNA)
            }
            
            row_idx = sample(which(na_vec == FALSE), size = min(beta_nCpG, length(which(na_vec == 
                FALSE))), replace = FALSE)
            
            lapply(X = seq_len(nrow(colData(meth))), FUN = function(i) {
                
                i_dens = density(get_matrix(meth, type = "M")[row_idx, i], na.rm = TRUE)
                data.table::fwrite(x = data.table::data.table(x = i_dens$x, y = i_dens$y), 
                  file = paste0(output_dir, "/", rownames(colData(x = meth))[i], 
                    "_density.tsv.gz"), sep = "\t")
    ...
    rmarkdown::render(input = md, output_file = "methrix_reports.html", output_dir = output_dir, 
        clean = TRUE, params = list(n_covered_tsv = of3, n_covered_by_all_samples_tsv = of4, 
            mc_per_chr_stat = of1, mc_per_sample_stat = of2, chr_lens = of5))
