  ...
  if(file.exists(of3)){
    ...
    if(recal_stats){
      non_cov_tbl = lapply(seq_len(ncol(meth)), function(i){
        data.table::as.data.table(as.data.frame(table(rowData(meth)[which(is.na(get_matrix(m = meth, "M")[,i])), "chr"])))
      })
      names(non_cov_tbl) = colnames(meth)
      gc()
      non_cov_tbl = data.table::rbindlist(l = non_cov_tbl, use.names = TRUE, fill = TRUE, idcol = "Sample_Name")
      ...
      non_cov_tbl = data.table::melt(meth@metadata$descriptive_stats$n_cpgs_covered, id.vars = 'chr')
